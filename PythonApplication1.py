...
def get_summary():
	...
	final_text = numpy.argmax(model.predict(raw_text), axis=-1)
	print(final_text)
...
def openfiles():
	...
	#read_text[['ADDRESS','OPCODE']]=read_text[['ADDRESS','OPCODE']].astype('float')
	#read_text['ADDRESS']=read_text['ADDRESS'].astype('float')
	#read_text['OPCODE']=read_text['OPCODE'].astype('float')
	#read_text[['ADDRESS','OPCODE']] = read_text[['ADDRESS','OPCODE']].apply(pd.to_numeric, errors='coerce').fillna(0.0)
...
def predict(code):
  ...
  df[['ADDRESS','OPCODE']] = df[['Amount','Costs']].apply(pd.to_numeric, errors='coerce').fillna(0.0)

  #code['OPCODE'] = pd.to_numeric(code['OPCODE'],errors = 'coerce').fillna(0.0)
  #code['ADDRESS'] = pd.to_numeric(code['ADDRESS'],errors = 'coerce').fillna(0.0)
  classtype1=np.argmax(model.predict(df), axis=-1)
  final_text =classtype1
  result = '\nSummary:{}'.format(final_text)
...
def get_file_summary():
	...
	tab2_display_text.insert(tk.END,df)
...
# About TAB
about_label = Label(tab5,text="Solution GUI V.0.0.1 \n ",pady=5,padx=5)
